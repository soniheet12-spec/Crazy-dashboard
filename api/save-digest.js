...
module.exports = async function handler(req, res) {
  ...
  if (typeof parsedBody === 'string') {
    try { parsedBody = JSON.parse(parsedBody); }
      ...
      console.error(`[save-digest] body is a string but not valid JSON: ${e.message}`);
      return res.status(400).json({ error: 'Request body is not valid JSON' });
    }
  }

  const { digest } = parsedBody || {};
  ...
  if (!base || !token) {
    console.error('[save-digest] Upstash env vars missing/invalid', {
  ...
  const target  = `${base}/set/pipeline_digest`;
  const payload = JSON.stringify(JSON.stringify(digest)); // Upstash stores as string
  ...
  try {
    ...
    console.log(`[save-digest] writing snapshot → ${target} (bytes=${payload.length})`);
    ...
    if (!r.ok || (parsed && parsed.error)) {
      console.error(`[save-digest] Upstash write failed status=${r.status} body=${text.slice(0, 300)}`);
      return res.status(502).json({ error: 'Upstash write failed', status: r.status, detail: (parsed && parsed.error) || text.slice(0, 300) });
    }
    return res.status(200).json({ ok: true, result: parsed && parsed.result });
  } catch (err) {
    console.error(`[save-digest] fetch threw url=${target} err=${err.name}: ${err.message}`);
